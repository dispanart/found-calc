...
type BillingStatusPayload = { billing: { entitlements: string[] } };
...
describe("Phase 07 billing API with D1", () => {
  ...
  it("correlates localized hosted checkout before provider return and activates only after authenticated webhook", async () => {
    ...
    const forgedReturnPayload = await forgedReturnStatus.json() as BillingStatusPayload;
    ...
    const statusPayload = await status.json() as BillingStatusPayload;
