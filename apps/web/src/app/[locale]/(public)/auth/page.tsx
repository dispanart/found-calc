...
type AuthPageProps = {
  ...
  searchParams: Promise<{ returnTo?: string | string[]; social?: string | string[] }>;
};
...
export default async function AuthPage({ params, searchParams }: AuthPageProps) {
  ...
  const returnTo = safeAuthReturnTo(rawReturnTo, locale);
  const socialCallback = query.social === "google";
  ...
  return (
    <main className="mx-auto grid w-full max-w-5xl gap-10 px-5 py-12 sm:px-8 sm:py-18 lg:grid-cols-[minmax(0,0.8fr)_minmax(22rem,1.2fr)] lg:items-start">
      <header className="min-w-0">
        <p className="text-xs font-semibold tracking-[0.12em] text-primary uppercase">Phase 04 + 07A</p>
        ...
        <p className="mt-5 max-w-xl text-base leading-7 text-muted-foreground">
          {locale === "id"
            ? "Kalkulator tetap dapat digunakan tanpa akun. Masuk hanya diperlukan saat Anda ingin membawa draft tersimpan ke identitas akun Anda."
            : "Calculators remain usable without an account. Sign in only when you want saved drafts attached to your account identity."}
        </p>
      </header>
      <AuthPanel locale={locale} returnTo={returnTo} socialCallback={socialCallback} />
    </main>
  );
}
