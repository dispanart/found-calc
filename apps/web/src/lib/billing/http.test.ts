...
const baseRepo = (): BillingHttpRepository => ({
  ...
  markCancellationRequested: async () => true, getEventOwner: async () => null, applyWebhookTransition: async () => ({ duplicate: false, applied: true, matched: true }),
});
const services = (overrides: Partial<BillingHttpServices> = {}): BillingHttpServices => ({
  ...
  xendit: { createSubscriptionSession: async (input) => ({ paymentSessionId: "ps-1", recurringPlanId: "rp-1", referenceId: input.referenceId, paymentLinkUrl: "https://payments.xendit.co/session/ps-1" }), deactivateSubscription: async () => undefined },
...
describe("billing HTTP boundary", () => {
  ...
  it("does not let cancellation input select provider identity", async () => {
    const sub = { id: "sub-1", userId: "user-1", planId: "fixture-pro", providerPlanId: "rp-secret", referenceId: "ref", status: "active" as const, latestCycleStatus: null, latestEventAt: 1, nextCycleAt: null, cancellationRequestedAt: null };
    let deactivated = "";
  ...
  it("authenticates webhook before parsing and validates amount/currency against local plan", async () => {
    let applied = 0;
    const repo = { ...baseRepo(), getEventOwner: async () => ({ userId: "user-1", planId: "fixture-pro" }), applyWebhookTransition: async () => { applied++; return { duplicate: false, applied: true, matched: true }; } };
