...
export function DiscountCalculator({ locale, entry }: DiscountCalculatorProps) {
  ...
  useEffect(() => {
    ...
    if (draft?.calculatorId === "reference.discount") {
      setBaseAmount(localize(draft.fields.baseAmount, draft.locale, locale, 2));
      setDiscounts(draft.fields.discounts.map((value) => localize(value, draft.locale, locale, 4)));
    }
    setLocalReady(true);
  }, [entry.id, locale]);
  ...
  }, [baseAmount, discounts, localReady, locale]);

  const dirty = () => setPersistableState(null);
  const setDiscount = (index: number, value: string) => {
    dirty();
    setDiscounts((current) => current.map((discount, currentIndex) => currentIndex === index ? value : discount));
  };
  const removeDiscount = (index: number) => {
    dirty();
    setDiscounts((current) => current.filter((_, currentIndex) => currentIndex !== index));
  };
  ...
  const calculate = () => {
    ...
    discounts.forEach((discount, index) => {
      ...
      if (!parsed.ok) {
        nextErrors[`discountPercentages.${index}`] = parsed.code === "empty" ? text.required : text.invalid;
        return;
      }
      normalizedDiscounts.push(parsed.value);
    });
    if (!parsedBase.ok || Object.keys(nextErrors).length > 0) {
    ...
    if (!nextOutcome.ok) {
      nextOutcome.issues.forEach((issue) => {
        const path = issue.path.replace(/\[(\d+)\]/g, ".$1");
        nextErrors[path] = issue.code === "out-of-range" ? text.range : text.invalid;
      });
      setFieldErrors(nextErrors); setOutcome(null); setPersistableState(null); return;
    }
    setFieldErrors({});
    ...
    setPersistableState({
      ...
      calculatorVersion: discountCalculatorDefinition.version.version,
      input: { baseAmount: parsedBase.value, discountPercentages: normalizedDiscounts },
    });
  };
        ...
        <div className="mt-6 space-y-5">
          ...
          <div className="space-y-4">
            {discounts.map((discount, index) => {
              const fieldKey = `discountPercentages.${index}`;
              const label = `${copy.fields.discountPercentages ?? ""} ${index + 1}`;
              return <div key={index} className="grid min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
                <CalculatorField id={`discount-step-${index + 1}`} label={label} value={discount} onChange={(event) => setDiscount(index, event.target.value)} inputMode="decimal" autoComplete="off" error={fieldErrors[fieldKey]} />
                {discounts.length > 1 ? <Button type="button" variant="ghost" onClick={() => removeDiscount(index)} aria-label={`${copy.ui.removeDiscount ?? "Remove discount"}, ${index + 1}`}>{copy.ui.removeDiscount}</Button> : null}
              </div>;
            })}
          </div>
          <div className="flex flex-wrap gap-3">
            <Button type="button" variant="outline" onClick={() => { dirty(); setDiscounts((current) => [...current, ""]); }}>{copy.ui.addDiscount}</Button>
      ...
      <div className="min-w-0">
        {outcome?.ok ? <ResultPanel title={text.result}>
          ...
          <dl className="mt-6 grid min-w-0 gap-4 border-t border-primary/15 pt-5 sm:grid-cols-2">
            {outcome.result.sections.flatMap((section) => section.values).map((value) => {
              const resultId = value.id.split(".")[0] ?? value.id;
              return <div key={value.id} className="min-w-0"><dt className="text-xs font-semibold text-trust-foreground/70">{copy.results[resultId] ?? resultId}</dt><dd className="mt-1 break-words text-lg font-bold text-trust-foreground">{formatCanonicalDecimal(value.value, locale, { style: value.unit === "percent" ? "percent" : "decimal" })}</dd></div>;
            })}
          </dl>
        </ResultPanel> : <div className="rounded-[var(--radius-card)] border border-dashed border-border bg-card/50 p-6 text-sm leading-6 text-muted-foreground">{locale === "id" ? "Masukkan urutan diskon lalu hitung untuk melihat hasil." : "Enter the discount order, then calculate to see the result."}</div>}
      </div>
    </div>
  );
}
