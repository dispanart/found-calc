...
export function DiscountCalculator({ locale, entry }: DiscountCalculatorProps) {
  ...
  const calculate = () => {
    ...
    if (!parsedBase.ok) {
      nextErrors.baseAmount = parsedBase.code === "empty" ? text.required : text.invalid;
    }
    ...
    const nextOutcome = runDiscount({
      baseAmount: parsedBase.value,
      discountPercentages: normalizedDiscounts,
    });
        ...
        <div className="mt-6 space-y-5">
          ...
          <div className="space-y-4">
            {discounts.map((discount, index) => {
              ...
              return (
                <div key={index} className="grid min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
                  <CalculatorField
                    id={`discount-step-${index + 1}`}
                    label={label}
                    value={discount}
                    onChange={(event) => setDiscount(index, event.target.value)}
                    inputMode="decimal"
                    autoComplete="off"
          ...
          <div className="flex flex-wrap gap-3">
            <Button type="button" variant="outline" onClick={() => setDiscounts((current) => [...current, ""])}>
              {copy.ui.addDiscount}
            </Button>
      ...
      <div className="min-w-0">
        {outcome?.ok ? (
          ...
          <div className="rounded-[var(--radius-card)] border border-dashed border-border bg-card/50 p-6 text-sm leading-6 text-muted-foreground">
            {locale === "id" ? "Masukkan urutan diskon lalu hitung untuk melihat hasil." : "Enter the discount order, then calculate to see the result."}
          </div>
        )}
      </div>
    </div>
