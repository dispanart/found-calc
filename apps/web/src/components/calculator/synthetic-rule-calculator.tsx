...
import { fetchPublishedRuleVersions, type PublishedSyntheticRuleVersion } from "@/lib/rules/client";
...
const textByLocale = {
  id: { summary: "Periksa input berikut.", required: "Kolom ini wajib diisi.", invalidNumber: "Masukkan angka yang valid.", range: "Nilai berada di luar rentang yang diizinkan.", invalidDate: "Masukkan tanggal efektif yang valid.", unavailable: "Tidak ada versi aturan terpublikasi untuk tanggal efektif ini.", ambiguous: "Tanggal efektif ini cocok dengan lebih dari satu versi aturan terpublikasi.", result: "Hasil referensi aturan", ongoing: "dan setelahnya", ruleFeedLoading: "Memuat versi aturan terpublikasi…", ruleFeedUnavailable: "Versi aturan terpublikasi sedang tidak tersedia. Perhitungan referensi dinonaktifkan sampai data dapat dimuat.", readyHint: "Masukkan nilai dasar dan tanggal efektif untuk memilih versi aturan terpublikasi secara eksplisit." },
  en: { summary: "Check the following inputs.", required: "This field is required.", invalidNumber: "Enter a valid number.", range: "The value is outside the allowed range.", invalidDate: "Enter a valid effective date.", unavailable: "No published rule version is available for this effective date.", ambiguous: "This effective date matches more than one published rule version.", result: "Rule reference result", ongoing: "and later", ruleFeedLoading: "Loading published rule versions…", ruleFeedUnavailable: "Published rule versions are currently unavailable. Reference calculation is disabled until the data can be loaded.", readyHint: "Enter a base amount and effective date to select a published rule version explicitly." },
} as const;
const localize = (value: string, from: Locale, to: Locale) => { if (from === to) return value; const parsed = parseLocaleDecimal(value, from, 2); return parsed.ok ? formatCanonicalDecimal(parsed.value, to) : value; };
...
function SyntheticRuleCalculatorStateful({ locale, entry, recordId }: SyntheticRuleCalculatorProps) {
  ...
  const [baseAmount, setBaseAmount] = useState(() =>
    initialDraft === null
      ...
      : localize(initialDraft.fields.baseAmount, initialDraft.locale, locale),
  );
  ...
  }, [baseAmount, effectiveDate, locale, surface.surface]);
  useEffect(() => {
    const controller = new AbortController();
    fetchPublishedRuleVersions(SYNTHETIC_RATE_RULE_ID, controller.signal).then((versions) => { setRuleVersions(versions); setRuleFeedStatus("ready"); }).catch(() => { if (controller.signal.aborted) return; setRuleVersions([]); setRuleFeedStatus("error"); });
  ...
  const calculate = () => {
    ...
    if (!parsedBase.ok || effectiveDate.trim().length === 0 || ruleFeedStatus !== "ready") { setFieldErrors(errors); setOutcome(null); setPersistableState(null); return; }
    ...
    setPersistableState({ calculatorId: "reference.synthetic-rule", calculatorVersion: syntheticRuleCalculatorDefinition.version.version, input: { baseAmount: parsedBase.value, effectiveDate } });
  ...
  const loadPersisted = (state: PersistedCalculatorState) => {
    ...
    setBaseAmount(formatCanonicalDecimal(state.input.baseAmount, locale)); setEffectiveDate(state.input.effectiveDate);
  ...
  const emptyState = ruleFeedStatus === "loading" ? text.ruleFeedLoading : ruleFeedStatus === "error" ? text.ruleFeedUnavailable : text.readyHint;
  return (
    <div className="grid min-w-0 gap-8 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] lg:items-start">
      <section className="min-w-0 rounded-[var(--radius-card)] border border-border bg-card p-5 sm:p-7">
        ...
        <div className="mt-6 space-y-5">
          {ruleFeedStatus !== "ready" ? <p className="rounded-lg border border-border bg-muted/40 px-4 py-3 text-sm leading-6 text-muted-foreground" role={ruleFeedStatus === "error" ? "alert" : "status"}>{ruleFeedStatus === "error" ? text.ruleFeedUnavailable : text.ruleFeedLoading}</p> : null}
