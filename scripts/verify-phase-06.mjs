...
const steps = [
  { label: "dependency-free Phase 06 contract tests", command: "pnpm", args: ["test:foundation"] },
  ...
  { label: "Cloudflare D1 workspace/rule/auth tests", command: "pnpm", args: ["--filter", "@found-calc/web", "test:cloudflare"] },
  ...
  {
    label: "complete Phase 05 regression gate",
    command: "pnpm",
    args: ["verify:phase05"],
...
for (const step of steps) {
  ...
  const result = spawnSync(step.command, step.args, {
    ...
    env: {
      ...process.env,
      BETTER_AUTH_SECRET: process.env.BETTER_AUTH_SECRET ?? "phase-06-ci-test-secret-not-for-production-000000000000000000000000",
      BETTER_AUTH_URL: process.env.BETTER_AUTH_URL ?? "http://127.0.0.1:3000",
      BETTER_AUTH_ADMIN_USER_IDS: process.env.BETTER_AUTH_ADMIN_USER_IDS ?? "phase-06-ci-admin-bootstrap",
    },
  });
  if (result.error) {
  ...
  if (result.status !== 0) {
    console.error(`Phase 06 verification stopped at ${step.label} (exit ${result.status ?? 1}).`);
    process.exit(result.status ?? 1);
  }
}

console.log("\nPhase 06 verification passed, including the complete Phase 05 regression gate.");
